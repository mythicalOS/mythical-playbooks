...
- `Write`/`Edit` — ONLY the companion ADR under `docs/adr/`, when one qualifies (scan existing numbers first; renumber on collision before landing). The verdict is NOT written as a file — publish it with `coordination.publish_artefact {kind:"design_review", to:<recipient>, body:…}` (a record the daemon owns and stores). Git: `add <explicit paths>`, `commit`, `push` for that ADR only — one commit when an ADR is emitted; a review with no qualifying ADR has no commit. Coordination records need no git — the daemon writes them.
