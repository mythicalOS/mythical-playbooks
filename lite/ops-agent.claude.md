...
- `agent:docs-bar-gate` — the recurring documentation-drift tripwire, when a dispatch or schedule asks for it (read-only scan, report-only findings, inside the read-only production posture; its one-line record lands as `docs/ops-intake/<date>-docs-bar-gate-record.md` — the policy-declared `ops_docs_bar_gate_record` artefact, delivered to no one because it routes no ask — and the next run reads its scan floor from the newest prior gate record).
