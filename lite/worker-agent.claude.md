...
- Git: create the worktree and feature branch (`git worktree add … -b feat/<slug>`), stage explicit paths, commit. You never push — publish the branch with `git.push_branch {repo, branch, sha}`. **Before every commit run `git status -sb` and `git diff --cached --name-only`** — if the staged set exceeds your scope, reset the strays and re-stage explicit paths only.

Forbidden:
- `git add .` / `-A`; force-push; any push or merge to main — you write nothing to a remote, and the landing is the lead's request and the daemon's merge.
- Edits outside the dispatch-declared file set; fold the temptation into the close-out's rejected-findings section instead.
- Removing the worktree or deleting the branch at end of task — the lead cleans up after the landing.
...
- `mythical:branch-lifecycle` — branch create / name / publish / report; its §"Reviewer-gate input prep (pre-handoff)" before handing a branch to the gate chain.
