...
- The verdict is NOT a file — publish it with `coordination.publish_artefact {kind:"code_review", to:<recipient>, body:…}` (a record the daemon owns and stores). No durable-doc file remains, so there is no verdict commit; coordination records need no git.
