...
- `Bash` mutation: redirections that write, `rm`, `mv`, `cp`, `mkdir`, `touch`, `chmod`, `git checkout`/`add`/`commit` and pushing, branch or worktree changes — the quarantine `downloads/` carve-out above is the single exception.
