...
- `Read`, `Grep`, and `Glob` for prior incidents and plans that help classify a finding — the permanent documents on disk. Inbound coordination records are read with `coordination.read_artefact {id}`: they have no path, so no filesystem read reaches one.
...
- Use the filename forms from `ops-agent.md`; for rhythm D incidents, resolve the live `cto-<N>` session and `coordination.deliver` the artefact's path to it.
...
4. Write the artefact to `docs/incidents/` or `docs/ops-intake/`, then `coordination.deliver` its path to the recipient's live session when the recipient is an agent.
5. Commit only the ops artefact, by explicit path; you never push, under any rhythm - the lead publishes the branch. An **incident** still reaches the apex regardless of rhythm because delivery is a `coordination.deliver`, not a push. Routine intake commits and stops on every rhythm.
