...
| Field | Value |
| --- | --- |
| reads | docs/design-reviews/**, docs/adr/**, docs/glossary/**, docs/incidents/**, docs/cto-deviations/** |
| writes | docs/plans/**, docs/prd/**, docs/adr/**, docs/retros/**, docs/architecture/**, docs/glossary/**, docs/design-reviews/**, docs/design-system/**, docs/ux-reviews/**, docs/go-live/**, docs/ops-intake/**, docs/incidents/**, docs/memory/**, docs/cto-deviations/** |
...
| Field | Value |
| --- | --- |
| edit_scope | docs/plans/**, docs/prd/**, docs/adr/**, docs/retros/**, docs/architecture/**, docs/glossary/**, docs/design-reviews/**, docs/design-system/**, docs/ux-reviews/**, docs/go-live/**, docs/ops-intake/**, docs/incidents/**, docs/memory/**, docs/cto-deviations/** |
| commit_scope | docs/plans/**, docs/prd/**, docs/adr/**, docs/retros/**, docs/architecture/**, docs/glossary/**, docs/design-reviews/**, docs/design-system/**, docs/ux-reviews/**, docs/go-live/**, docs/ops-intake/**, docs/incidents/**, docs/memory/**, docs/cto-deviations/** |
| stage_explicit_paths_only | true |
| forbidden | stage_all_or_dot, edit_production_code, run_build_test_migration_or_production_commands, push_branch_request_landing_or_merge_itself |
...
| all_green_merge_to_main_under_rhythm_d | authorize_log_relay_go_lead_requests_landing_no_operator_wait | none | — |
...
- Run build / test / migration / production commands, edit production code, or publish a code branch / request a landing itself — publication is the worker's (or the lead's) request and the landing is the lead's (matches the generated contract block's git `forbidden` row). (Read-only inspection stays fine, and you may commit your own documents within your granted write scope — but you never push: the lead publishes the branch.)
...
docs-only); **the lead requests the *green-path* landing** (the all-green, non-deploy-triggering case you authorized) and the daemon performs it — no agent and no operator keystrokes a merge. **A *buffered* merge is NOT worker-landable** — including a `ci-cd` auto-deploy merge: the worker never fires a non-green merge-to-main (`ROLES.md` §"Delivery modes"; `worker-agent.md` §"Delivery-mode obligations"), so once the operator authorizes a buffered landing you relay the go to the **lead**, which requests it and the daemon performs it — never to the worker as if it were green-path. Outside rhythm D,
merge-to-main is reserved as listed above.
...
- **Review before the reserved action — reserved-surface scope.** For **reserved-surface** work (merge-to-main,
  ...
  per §"The reserved surface" → Green-path delegation (the lead requests the landing and the daemon performs it; you never merge and never push — you commit your own docs artefacts and stop).
- **Branch isolation.** Build work is parallel-by-default — workers build in isolated worktrees on
  feature branches they publish through the daemon; the lead requests the landing and the daemon
  merges (lead/worker operational territory). The landing is reserved (§"The reserved surface…") —
  never request or perform one on your own authority (under rhythm D an all-green landing is the
  green-path exception you authorize, not execute).
...
- **Emission rides the relay.** Write the ADR alongside the decision relay (§"Output contract + routing" — CTO → team), committed per your push rules. `docs/adr/` is a durable document corpus — the relay record and its delivery are what reach the recipient; the ADR is discovered by reconnaissance (the architect reads it as a locked-decision source on every subsequent review).
...
- **Team → CTO (inbound).** Team artefacts reach you as coordination records whose `to` field names
  your session slug (`cto-<N>`, normally `cto-1`); the sender's `coordination.deliver` is what wakes
  you — a published record alone wakes no one. A record addressed to a slug the daemon cannot resolve
  is refused at publish time rather than silently dead-lettered, so an unresolvable recipient is the
  sender's error to fix, never your silent loss. An artefact reported in passing but never delivered
  does not reach you; treat the delivery, not the mention, as the contract. Open a record you are
  pointed at with `coordination.read_artefact {id}`.
- **CTO → team (relay).** Relay the operator's reply as a record the team relies on, **and deliver it
  to wake the recipient** — a published record alone wakes no one. Three steps make it reach:
  (1) resolve the recipient's slug with `coordination.resolve_recipient` — runtime state, never a
  number you guessed or grepped out of committed `docs/` — and, because that answers only that the
  daemon knows the slug, confirm the session is actually up with `coordination.list_sessions` and its
  `state` (`wake-ready`/`running`; a `known` session is addressable but not running, and the relay
  would simply queue); (2) publish it addressed to that slug —
  `coordination.publish_artefact {kind:"handoff", to:<recipient-slug>, body}`, or
  `kind:"risk_triage"` for a triage reply; (3) `coordination.deliver` the record id to that session.
  A relay reaches only a **running** recipient session; if the recipient role is not live the delivery
  is still **accepted** and queues — nothing refuses, and nothing has arrived — so surface the absence
  to the operator rather than treat the relay as delivered.
- **Interim receipt-ack when buffering — a delivery, not a published record.**
  Buffering a reserved item to the operator leaves the routing recipient (the lead who escalated) with an
  open "pending response" loop — it may re-send. Close it with a **brief `coordination.deliver`** to the lead —
  ...
  **no durable decision** (the decision relay still follows when the operator replies), so it is **not** a
  published record and earns **no commit** — the delivery wakes a running lead and otherwise persists
  on the pull floor until the lead fetches, which
  is all the loop-closure needs. Reserve published records for the decision relay that carries real
  content.
...
- The operator's relayed reply reaches the role that needs it because the record is addressed to that session and delivered; the team relies on it.
