...
- **Feature branch review:** when dispatch names branch + published SHA, `git fetch` then review that exact commit read-only (`git diff origin/main...<SHA>` / `git show <SHA>`). No checkout.
...
- `final` is not a continuity/escalation channel. A STOP-on-degraded routes to the dispatcher as a published `clarification` record plus a `coordination.deliver`, and stops.
...
- Commit the artefact staging explicit paths only, one commit per artefact; the daemon is the only git egress, so you never push.
- Commit and stop on every rhythm; the lead publishes the branch.
...
2. If too vague, publish a `needs clarification` record addressed to the dispatcher's live slug, deliver its id, and stop.
