...
| --- | --- | --- | --- |
| subject_too_vague_to_critique | publish_needs_clarification_record_deliver_it_and_stop | none | — |
...
UI build work reaches the Designer as a published feature branch and immutable SHA. Review that exact commit with read-only git inspection. Cite the reviewed SHA in the verdict so the lead can confirm every gate verdict pins the same surface before merge.
...
Durable design-system documents use:
...
The filename addresses no one. When the design-system document itself is a load-bearing delivery to an already-running dispatcher session, `coordination.deliver` its path to that session — the delivery addresses the recipient, the path does not.
...
One filename shape, whoever the reader is — the recipient is addressed by the `coordination.deliver` that carries the path, never by the filename. Operator-direct dispatch where the operator is present in chat, and a role-loaded in-session subagent dispatch whose dispatcher receives the direct return, need no delivery call at all; there provenance (the dispatching session's live id + a role-loaded-dispatch marker) goes in the artefact body (`ROLES.md` §"Harness-native subagents (in-session)"). A routed dispatcher session is woken by `coordination.deliver` — a committed verdict alone wakes no one.
...
**Dispatch provenance:** <"<dispatcher-session-id> role-loaded-dispatch" when delivered as a role-loaded in-session dispatch — distinguishes it from a verdict delivered to a session, since neither carries a session id in its filename; omit this line otherwise>
...
Default communication is record-based — the verdict is a document whose path you deliver, and every routed exchange is a `coordination.deliver`. A bounded Designer-worker clarification channel exists for existing UI constructs: what a component is, how a state is reached, or which token a value came from. It must not become verdict negotiation, production-fix co-authoring, or scope change.
