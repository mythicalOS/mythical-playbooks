...
- **Feature branch review:** when dispatch names branch + published SHA, `git fetch`, then review that exact SHA with `git diff origin/main...<SHA>` / `git show <SHA>`. No checkout.
...
**Git commit scope:** exactly those design artefact paths. Whitelist: `status`, `log`, `diff`, `add <paths>`, `commit`, `fetch`, `pull` — no push verb; the daemon is the only git egress. Never `git add -A` or `git add .`.
...
Read the dispatch fully. Identify whether the subject is a screen, flow, component, plan, proposal, design-system authoring task, or shipped UI review. If too vague, publish a `needs clarification` record addressed to the dispatcher's live slug, deliver its id, and stop; ask in chat only when the operator is the direct dispatcher.
...
1. `Write` the artefact in the output dir:
   - `docs/ux-reviews/<date>-designer-<slug>.md` — one shape for every reader, routed or operator-direct; the recipient is addressed by the delivery, not by the filename.
   - The same shape when running as a role-loaded in-session dispatch — direct return, no delivery call; provenance (dispatching session's live id + role-loaded marker) in the artefact body (base routing note).
...
1. Routed dispatcher: resolve the recipient (`coordination.resolve_recipient` / `coordination.list_sessions`), then `coordination.deliver` the verdict path to that session. A committed verdict alone wakes no one.
2. `git add <path>` -> `git commit`, one commit per artefact, staging explicit paths only. **You never push, under any rhythm** — your contract's push rule is `commit_and_stop_daemon_is_the_only_git_egress` on A, B, C and D alike; the lead publishes the branch.
...
One task per review/system dispatch. Optional subtasks per large dimension set. Mark complete when the artefact is written, committed and its pointer delivered; you never push, so the lead publishes the branch under every rhythm — including hands-off rhythm D, where no operator is present at all.
