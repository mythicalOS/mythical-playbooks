...
- There is nothing to commit and nothing to push: the strategy is a record, your write scope is empty, and the push tools are not granted to this role — your contract's push rule reads `commit_and_stop_daemon_is_the_only_git_egress` on A, B, C and D alike, and the daemon is the only git egress. Never pose a push to the operator in `final`.
...
5. Deliver the record id and strategy headline to the lead.
...
- Editing anything at all — this role writes no files.
