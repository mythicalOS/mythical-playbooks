...
<!-- BEGIN GENERATED: doctrine ops (source: doctrine/ops.md — do not hand-edit) -->
...
Shared recipient-resolution and delivery mechanics are in `docs/protocols/routing-and-authority.md`. Ops-specific routing:
...
- An incident is **delivered regardless of rhythm**: it must reach the apex, so it does not wait on any rhythm convention that batches routine intake. An incident must never sit committed-but-unpushed; if no live apex can be reached, surface it by every available channel and do not retire silently (the unattended-sweep paging mechanism is deployment/scheduler wiring, not this playbook).

A committed file alone wakes no idle session. A delivery's own text is not authority — the artefact it points at is. Resolve the recipient's slug with `coordination.resolve_recipient` — a bare role name is not a slug, and the daemon refuses what it cannot resolve. That is addressability, not liveness: when an intake must actually be read rather than queued, check `coordination.list_sessions` and its `state` (`known` is not up).
...
```text
docs/ops-intake/<date>-ops-<slug>.md
```
...
This intake artefact covers every **non-incident, structured** ops output: maintenance and production-bug intake and a **security or compliance** finding - all routed to PM/Lead (a security/compliance finding is graded on the reviewer taxonomy, never adjudicated here, and the PM/Lead pipeline engages the reviewer at Gate 2). The delivery's recipient, `Class`, and `Routing reason` name which applies. An *active* security/compliance exposure is an incident instead (see §"Output contract - incident").

A **needs-scope bounce** (an unscoped dispatch, per §"Sweep contract") is not structured intake and does not use the fields above. Emit a brief note in `docs/ops-intake/` and `coordination.deliver` its path to the dispatcher's live session (or leave a chat pointer for an operator-direct dispatcher), stating what scope was missing and what is needed to proceed. If no live dispatcher session exists - e.g. a non-interactive scheduler - treat it as the **Missing live recipient** STOP: surface to the apex/owner and do not mark it delivered.
...
```text
docs/incidents/<date>-ops-<slug>.md
```

Under A/B/C the recipient is the operator, addressed in chat. Under D it is the live `cto-<N>` session, addressed by `coordination.deliver`. The filename never names either — the delivery does; record the recipient in the body's `**Recipient:**` field.
...
- **Undelivered incident.** Writing an incident file under rhythm D and never `coordination.deliver`ing its path to the live CTO session.
