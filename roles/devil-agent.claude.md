...
- `Bash` mutation: output redirection that writes, `rm`, `mv`, `cp`, `mkdir`, `touch`, `chmod`, the mutating `git` verbs (`checkout`, `add`, `commit`, and pushing), branch or worktree creation/removal — the quarantine `downloads/` carve-out above is the single exception.
