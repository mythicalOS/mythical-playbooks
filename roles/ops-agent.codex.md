...
- For routed agent recipients, the filename must carry the live numbered `-to-<recipient-id>-` token and must be paired with a bus wake. `final` to the operator is not delivery to an idle routed agent session (PM, Lead, or CTO).
...
- Do not push production code or configuration. Ops may commit/push its own artefacts only when the active routing permits it. An **incident** self-pushes regardless of rhythm so it reaches the shared remote; routine intake under rhythm D commits and stops for the single pusher.
...
5. Deliver the path and one-line classification. For an idle agent recipient, delivery requires bus wake in addition to the file.
...
- Writing a tokenless `docs/incidents/` file under rhythm D and assuming the CTO saw it.
