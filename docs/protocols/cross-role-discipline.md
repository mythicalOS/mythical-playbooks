...
not wired). The per-role mapping is in
`distillation-notes/cross-role-session-friction-distillation-2026-06-24.md`
(raw per-role rules in Part B; the promotion/wiring record is Part C). Carve-outs
...
Persisting work is not delivering it, and a registry is not liveness. Roles
assumed a committed artifact or an in-channel reply had notified a recipient
(it had not), and dispatched to a seat assumed alive because it "answered
earlier" or appeared in a registry file, when the roster had since rotated —
work then stranded silently.

Rule: separate *persisting* (commit, doc) from *notifying* (an explicit message
on the channel the recipient actually consumes) — committed is not
communicated. Re-verify recipient liveness from ground truth immediately before
each dispatch, never from past responsiveness or a self-reported registry. A
dispatched task is not a progressing task: track it, sweep for
finished-but-unrouted or stuck work, and re-wake.

The full delivery contract (watched directory, live numbered recipient, bus
wake) is in `docs/protocols/routing-and-authority.md`. This section is the
generalized discipline behind it.
...
become HEAD; a push carried unintended commits sitting beneath the intended one;
a dependency was pushed without bumping its pointer (or the reverse); a forced
stop left a partial push.

Rule: prefer forward-only new commits over amend/rebase on shared branches.
Before pushing, inspect the full range being sent (`git log @{u}..HEAD`) and
confirm every commit is intended — never push whatever happens to sit beneath
the target. Treat coupled multi-repo or submodule commits as atomic: land the
dependency and bump its pointer in the same logical step. On a forced stop or
degradation, return to a consistent committed or unstarted state, never a
partial one.
